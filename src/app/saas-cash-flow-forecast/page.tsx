      ...
      <section className="px-4 sm:px-6 py-16 sm:py-20">
        <div className="mx-auto max-w-5xl">
          ...
          <p className="text-gray-400 text-sm sm:text-base mb-10 sm:mb-14">
            From Stripe connection to 12-month forecast in under 5 minutes.
          </p>
          <div className="space-y-0 divide-y divide-white/8 border-y border-white/8">
            {STEPS.map((step) => (
              <div
                key={step.n}
                className="grid grid-cols-[2.5rem_1fr] sm:grid-cols-[3rem_1fr] gap-4 sm:gap-6 py-5 sm:py-6 items-start"
              >
                <div className="text-xl sm:text-2xl font-bold text-white/15 tabular-nums pt-0.5">
                  {step.n}
                </div>
                <div>
      ...
      <section className="px-4 sm:px-6 py-16 sm:py-20 bg-white/[0.02] border-y border-white/8">
        <div className="mx-auto max-w-3xl">
          ...
          <p className="text-gray-400 text-sm sm:text-base mb-10">
            Full cash flow forecasting at a fraction of the cost.
          </p>
            ...
            <table className="w-full text-sm min-w-[440px]">
              <thead>
                <tr className="border-b border-white/10">
                  ...
                  <th scope="col" className="text-center py-3 pb-4 text-gray-500 font-medium">
                    Baremetrics
                  </th>
                  <th scope="col" className="text-center py-3 pb-4 text-gray-500 font-medium">
                    ChartMogul
              ...
              <tbody>
                {COMPARISON.map((row, i) => (
                  <tr
                    key={row.feature}
                    className={`border-b ${
                      i === COMPARISON.length - 1 ? 'border-transparent' : 'border-white/5'
                    }`}
                  >
            ...
            {FAQS.map((faq) => (
              <details key={faq.q} className="group py-4 sm:py-5">
                <summary className="flex items-start justify-between gap-4 cursor-pointer list-none min-h-[44px]">
                  ...
                  <span
                    className="text-gray-500 mt-0.5 shrink-0 transition-transform group-open:rotate-180"
                    aria-hidden="true"
                  >
