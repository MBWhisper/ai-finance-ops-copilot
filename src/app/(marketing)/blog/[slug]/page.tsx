...
export default async function BlogPost({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { content } = await compileMDX({
    ...
    components: {
      ...
      a: (props) => {
        ...
        const cls = "text-emerald-400 hover:text-emerald-300 underline underline-offset-2"
  ...
  const readTime = Math.max(1, Math.round(mdxContent.split(/\s+/).length / 200))
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100">
      ...
      <article className="mx-auto max-w-3xl px-6 py-24">
        <div className="mb-8">
          <Link href="/blog" className="inline-flex items-center gap-1 text-sm text-gray-400 hover:text-emerald-400 transition-colors mb-6">
            &larr; Back to Blog
          </Link>
          <div className="flex items-center gap-3 mb-4">
            {frontmatter.tags?.slice(0, 2).map((tag: string) => (
              <span key={tag} className="inline-block rounded-full bg-emerald-500/10 px-3 py-1 text-xs font-medium text-emerald-400 capitalize">
                {tag.replace(/-/g, ' ')}
              </span>
            ))}
          ...
          <h1 className="text-4xl font-bold text-white mb-4 leading-tight">{frontmatter.title}</h1>
          <p className="text-lg text-slate-400 leading-relaxed">{frontmatter.description}</p>
        </div>

        {/* Cover image — always from getCoverImage, never broken */}
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
          src={coverUrl}
        ...
        <div className="mt-16 border-t border-slate-800 pt-10 text-center">
          <h3 className="text-xl font-bold text-white mb-3">Track Your SaaS Metrics for Free</h3>
          <p className="text-slate-400 mb-6">AI Finance Ops automatically tracks MRR, churn, runway, and more.</p>
          <Link href="/register" className="inline-flex items-center gap-2 rounded-lg bg-emerald-500 px-6 py-3 text-sm font-semibold text-white hover:bg-emerald-400 transition-all">
